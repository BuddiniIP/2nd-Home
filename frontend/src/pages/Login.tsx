  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white px-4 sm:px-6 pb-16 sm:pb-24">
      <motion.div 
        ...
        className="flex flex-col lg:flex-row bg-white rounded-[2rem] sm:rounded-[3.5rem] shadow-sm overflow-hidden max-w-4xl w-full min-h-0 sm:min-h-[550px]"
      >
        {/* Image Banner - compact top on mobile, full left side on desktop */}
        <div className="relative w-full lg:w-1/2 h-44 lg:h-auto overflow-hidden flex-shrink-0">
           <img 
             src="/images/house_white.png"
             alt="Login" 
             className="absolute inset-0 w-full h-full object-cover"
           />
           <div className="absolute inset-0 bg-black/30" />
           <div className="absolute inset-0 flex flex-col justify-center px-6 lg:px-12 text-white">
              <h2 className="font-display text-xl lg:text-4xl font-bold mb-1 lg:mb-4">Welcome Back.</h2>
              <p className="text-white/70 text-xs lg:text-sm leading-relaxed">Sign in to continue managing your 2nd Home experience.</p>
           </div>
        </div>

        {/* Right Side: Form */}
        <div className="flex-1 p-6 sm:p-12 md:p-16 flex flex-col justify-center">
          {/* Tab Switcher */}
          <div className="flex gap-0 mb-10 bg-gray-50 rounded-full p-1 w-full sm:w-fit">
            <button
              ...
              >
                ...
                <form className="space-y-6" onSubmit={handleLogin}>
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="email" placeholder="name@example.com" value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-gray-50 border border-transparent focus:border-accent-orange focus:bg-white transition-all rounded-full px-6 py-4 text-sm outline-none" required />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase font-bold tracking-widest text-gray-400 px-4">Password</label>
                    <input type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} className="w-full bg-gray-50 border border-transparent focus:border-accent-orange focus:bg-white transition-all rounded-full px-6 py-4 text-sm outline-none" required />
                  </div>
                ...
                {!codeSent ? (
                  <div className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <input type="email" placeholder="name@example.com" value={resetEmail} onChange={e => setResetEmail(e.target.value)} className="w-full bg-gray-50 border border-transparent focus:border-accent-orange focus:bg-white transition-all rounded-full px-6 py-4 text-sm outline-none" required />
                    </div>
                  ...
                  <form className="space-y-6" onSubmit={handleResetPassword}>
                    <div className="space-y-2">
                      ...
                      <input type="text" placeholder="e.g. 48291735" maxLength={8} value={resetCode} onChange={e => setResetCode(e.target.value.replace(/\D/g, '').slice(0, 8))} className="w-full bg-gray-50 border border-transparent focus:border-accent-orange focus:bg-white transition-all rounded-full px-6 py-4 text-sm outline-none text-center text-2xl tracking-[0.3em] font-bold" required />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase font-bold tracking-widest text-gray-400 px-4">New Password</label>
                      <input type="password" placeholder="Min. 6 characters" value={newPassword} onChange={e => setNewPassword(e.target.value)} className="w-full bg-gray-50 border border-transparent focus:border-accent-orange focus:bg-white transition-all rounded-full px-6 py-4 text-sm outline-none" required />
                    </div>
                    <div className="space-y-2">
